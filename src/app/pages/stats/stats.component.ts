...
@Component({
  selector: 'tc-stats', templateUrl: './stats.component.html', styleUrls: ['./stats.component.scss'],
})
export class StatsComponent implements OnInit {
  ...
  data: {
    created: {
      sooner: number; later: number; 'maybe never': number; all: number;
    }; resolved: {
      sooner: number; later: number; 'maybe never': number; all: number;
    }; rejected: {
      sooner: number; later: number; 'maybe never': number; all: number;
  ...
  ngOnInit(): void {
    this.tasksService.getTasks('', [TaskStatus.Created, TaskStatus.Resolved, TaskStatus.Rejected]).subscribe((tasks) => {
      tasks.forEach((task) => {
